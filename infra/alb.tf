...
#
# ターゲットは ECS Fargate task の ENI:80 (target_type=ip)。
...
# Issue #61: non-root container では port 80 不可のため port 3000 に変更。
resource "aws_security_group_rule" "ecs_task_from_alb" {
  type                     = "ingress"
  description              = "Rails HTTP from ALB only (Thruster HTTP_PORT=3000)"
  from_port                = 3000
  to_port                  = 3000
  protocol                 = "tcp"
...
resource "aws_lb_target_group" "app" {
  name        = "${var.project}-tg"
  port        = 3000 # Issue #61: ECS task (non-root) 用 non-privileged port
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
