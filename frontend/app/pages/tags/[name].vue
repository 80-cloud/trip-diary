...
<template>
  <div>
    <NuxtLink to="/" class="text-sm text-brand-600 hover:underline">← タイムラインに戻る</NuxtLink>

    <div v-if="pending" class="text-center py-12 text-slate-500">読み込み中…</div>
    <div v-else-if="error && error.statusCode === 404" class="bg-white rounded-lg p-8 text-center border border-slate-200 mt-4">
    ...
    <template v-else-if="data">
      <header class="bg-white border border-slate-200 rounded-lg p-6 my-4 flex items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-brand-700">#{{ data.tag.name }}</h1>
          <p class="text-sm text-slate-500 mt-1">{{ data.tag.trips_count }} 件の旅行記録</p>
        </div>
      ...
      <div v-if="data.trips.length === 0" class="bg-white rounded-lg p-8 text-center border border-slate-200">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="p-4 flex-1 flex flex-col">
            ...
            <p class="text-xs text-slate-500 mt-1">{{ trip.started_on }} 〜 {{ trip.ended_on }} / {{ trip.destination }}</p>
            ...
            <div class="mt-2 flex flex-wrap gap-1">
              ...
              <span v-for="tname in (trip.tags || []).slice(0, 3)" :key="tname" class="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-600">#{{ tname }}</span>
            </div>
